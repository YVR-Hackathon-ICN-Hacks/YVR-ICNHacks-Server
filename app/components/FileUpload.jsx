...
import {
  getMeanAndSD,
  checkAbnormalData,
} from "../../lib/utils/checkAbnormalData";
...
const FileUpload = () => {
  ...
  const [isAreaCodeAndDateduplicated, setIsAreaCodeAndDateduplicated] =
    useState(false);
  ...
  const CcheckAbnormalData = async () => {
    ...
    const [avgT, sdT, avgAF, sdAF, avgCO2, sdCO2] = await getMeanAndSD(
      dataToUpload
    );
    ...
    for (const item of dataToUpload) {
      [
        ...
        abnormal_air_flow,
        abnormal_co2,
      ] = await checkAbnormalData(item, avgT, sdT, avgAF, sdAF, avgCO2, sdCO2);
      if (is_abnormal) {
        const abnormalData = {
